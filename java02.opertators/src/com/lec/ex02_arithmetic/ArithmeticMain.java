...
/*
	...
	산술연산자의 특징은 피연산자들의 타입이 동일타입이 아닐 경우 피연산자들의
	타입을 일치 시킨후에 연산을 수행한다. 타입을 일치시티는 경우 아래와 같은 
	규칙을 가지고 연산을 수행한다.
	...
	3. 피연산자중 실수타입(float, double)이 있을 경우 크기가 큰 실수 타입으로 변환후
	   연산을 수행한다.
	   
	   int + double -> double + double = 결과타입은 double이다.
	   
	간단하게 정리하면 long타입을 제외한 정수타입의 연산은 int타입으로 산출되고 피연산자중
	하나라도 실수타입이 있다면 결과는 실수타입으로 산출된다.
	
	정수타입의 연산결과가 int타입으로 나오는 이유는 "JVM이 기본적으로 32bit(4byte)단위로
	연산을 하기 때문이다.
