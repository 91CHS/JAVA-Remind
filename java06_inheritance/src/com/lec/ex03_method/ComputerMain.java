...
/*
   ...
   일부 메서드는 자식클래스에서 다시 수정해서 사용해야 한다. Java는 이런 경우를 위해서 
   메서드 오버라이드 기능을 제공한다.
	...
	부모메서드가 오버라이드 외에 있다면 부모객체의 메서드는 숨겨지기 때문에 자식객체에서 메서드를
	호출하면 오버라이드 된 자식객체의 메서드가 호출된다.
	
	메서드를 오버라이드할 경우에는 아래와 같은 규칙에 의거해서 작성해야 한다.
	1. 부모메서드와 동일한 시그니처(리턴타입,메서드이름,매개변수목록)를 가져야 한다.
	2. 접근제한을 부모객체보다 강하게 정의 할 수 없다.
	3. 새로운 예외(Exception)을 떠넘길 수 없다.(throws)
