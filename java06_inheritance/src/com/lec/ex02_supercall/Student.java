...
/*
   ...
   모든 객체는 클래스의 생성자를 호출해야만 생성할 수 있다.
   부모객체도 예외는 아니다.
   Java문법에서 부모객체를 생성하기 위해서는 자식생성자에서 부모생성자를 호출해야 한다.
   ...
   호출해야 한다.
 	...
 	부모클래스에 기본생성자가 없고 일번생성자(매개값이 있는)가 있다면 자식생성자에서는
 	반드시 부모생성자를 호출하기 위해서 동일 시그니처를 갖는 super(매개값...)생성자를
 	명시적으로 선언 및 호출해야 한다. 그리고 반드시 부모생성자의 호출은 자식생성자의
 	맨 처음에 위치해야 한다.
