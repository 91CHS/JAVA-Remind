...
/*
   ...
   직접 수정하지 않고 사용하는 객체만 변경할 수 있도록 하기 위해서 이다.
   
   인터페이스는 하나의 객체가 아니라 여러 객체들이 사용하기 때문에 어떤 객체를 사용하느냐에
   따라서 실행내용과 결과값이 다를 수 있어서 개발코드 측면에서 보면 코드의 변경없이 실행내용과
   리턴값을 다양화할 수 있다는 장점이 있다.
 */
public interface RemoteControl {

	// 1. 상수필드
	// 인터페이스는 객체의 설계도 역할을 하기 때문에 런타임시 데이터를 저장할 수 있는
	// 필드를 선언할 수 없다. 하지만, 상수는 인터페이스에 고정된 데이터로서
	// 런타임시에 변경할 수 없는 특성을 갖는다. 상수를 선언할 때는 반드시 초기화
	// 즉, 초기값을 대입해야 한다. 인터페이스에서 필드는 상수로 선언되기 때문에
	// 별도로 명시하지 않아도 컴파일시에 자동으로 public static final이 자동추가된다.
	...
	// 필요한지, 어떤 리턴값이 있는지 등을 정의한다. 실제 실행블럭은 객체(구현 or 실제객체)가
	// 가지고 있다. 인터페이스의 메서드는 무조건 추상메서드로 선언되기 때문에 생략하더라도
	// 컴파일시에 자동으로 public abstract가 추가된다.
	void powerOn();
	void powerOff();
	public abstract void cetVoulumn(int volumn);
	
	// 3. 디폴트메서드
	// default메서드는 인터페이스에 선언되지만 구현객체가 가지고 있는 메서드라고
	// 생각하면 된다. Java8버전에서 default메서드를 허용한 이유는 기존 인터페이스를
	// 확장해서 새로운 기능을 추가하기 위해서 이다. default메서드는 실행블럭을
	// 가지고 있는 메서드로서 default키워드를 반드시 붙여야 하며 기본적으로
	// public 접근제한자를 가지고 있고 생략하더라도 컴파일러가 자동으로 추가한다.
	default void setMute(boolean mute) {
	 ...
	 // 멤버로서 "인터페이스명.메서드명"형태로 호출할 수가 있다.
...
class TVRmocon implements RemoteControl {
	...
	@Override
	public void cetVoulumn(int volumn) {
