...
/*
	...
	문자열을 저장하는 string클래스는 내부의 문자열을 수정할 수 없다. 예를 들어서 String.replace()
	메서드는 내부의 문자여을 변경하는 것이 아니라 새롭게 대체된 문자열객체를 리턴한다.
	또한 예를 들어 "JAVA" + "PROGRAMMING"을 실행하는 경우 내부문자열을 수정할 수 없기 때문에
	"HAVA PROGRAMMING"이라는 새로운 String객체를 생성한 후에 그 객체의 참조번지를 리턴한다.
	문자열을 결합할 경우 "+"연결연산자를 사용하면 할 수록 그 사용수 만큼 새로운 String객체의 수가
	늘어나기 때문에 프로그램의 성능을 느리게하는 요인이 된다.
	...
	문자열을 저장해 두고 그 안에서 수정,추가,삭제 등의 작업을 할 수 있도록 한다. String처럼 새로운
...
public class StringBufferMain {
	...
	public static void main(String[] args) {
		...
		System.out.println(sb.hashCode() + ", " + sb);  // 해쉬코드 동일. 즉 추가된 것
