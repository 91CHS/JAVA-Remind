...
/*
 	...
 	객체가 단 한개만 생성된다고 해서 이러한 디자인패턴을 싱글톤 패턴이라고 한다.
 
 	싱글톤객체를 만드는 방법
 	
 	1. 외부에서 new객체생성연산자로 생성자를 호출할 수 없도록 막아야한다.
 	   그 이유는 생성자를 호출한 만큼 무제한으로 객체가 생성되기 때문이다.
 	   생성자를 외부에서 호출할 수 없도록 하려면 생성자에 접근을 하지 못 하도록
 	   private접근제한자로 선언하는 것이다.
...
//		 Singleton sing4 = new Singleton();
//		 Singleton sing5 = new Singleton();
//		 
		 ...
		 System.out.println(sing5.hashCode());
