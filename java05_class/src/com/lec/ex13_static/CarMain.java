...
/*
   ...
   또한, main메서드도 정적메서드이기 때문에 동일한 규칙이 적용된다. 이런
   이유로 main메서드가 객체생성없이 객체멤버필드와 메서드를 바로 사용할 수 없다.
...
public class CarMain {
		int speed;
		void run() {
			System.out.println("현재 자동차의 속도는" + speed + "km/h입니다!");
		}

	public static void main(String[] args) {
		
		//speed와 run() 메서드를 접근할 수 있다? 없다? => 없다
// 		speed = 100;
//		run();

//		speed와 run()는 객체멤버이기 때문에 정적멤버인 main에서 접근할 수 없다.
//		접근하려면 speed와 run()을 정적멤버로 즉, static으로 선언해야 한다.
//		또는 객체를 만들어줘야 한다.
